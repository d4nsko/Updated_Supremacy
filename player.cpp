...
void Hooks::DoExtraBoneProcessing(int a2, int a3, int a4, int a5, int a6, int a7) {
	...
	if (animstate) {
		// backup player ptr.
		backup = animstate->m_baseEntity;

		// null player ptr, GUWOP gang.
		animstate->m_baseEntity = nullptr;
	}
...
void Hooks::UpdateClientSideAnimation() {
	if (g_cl.m_update && g_cl.m_local && g_cl.m_local != nullptr)
...
Weapon* Hooks::GetActiveWeapon() {
	...
	static Address ret_1 = pattern::find(g_csgo.m_client_dll, XOR("85 C0 ? ? 8B 88 ? ? ? ? 85 C9 ? ? 80 39 00 ? ? 51 56"));
...
void Hooks::CalcView(vec3_t& eyeOrigin, vec3_t& eyeAngles, float& zNear, float& zFar, float& fov) {
	...
	auto m_bUseNewAnimstate = player->get<bool>(0x3AC8);

	player->set<int>(0x3AC8, false);

	g_hooks.m_CalcView(this, eyeOrigin, eyeAngles, zNear, zFar, fov);

	player->set<int>(0x3AC8, m_bUseNewAnimstate);
}
		...
		if (ent->IsPlayer()) {
			...
			if (vmt) {
				...
				if (player->index() == g_csgo.m_engine->GetLocalPlayer()) {
					...
					g_hooks.m_CalcView = vmt->add< Hooks::CalcView_t >(276, util::force_cast(&Hooks::CalcView));
				}
			}
		}
