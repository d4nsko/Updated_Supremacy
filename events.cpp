...
// points to safe delay between death/respawn
int iLife = false;
...
void events::weapon_fire(IGameEvent* evt) {
	...
	if (!strcmp(evt->GetName(), "weapon_fire"))
	{
		int shooter = g_csgo.m_engine->GetPlayerForUserID(evt->m_keys->FindKey(HASH("userid"))->GetInt());

		if (shooter == g_csgo.m_engine->GetLocalPlayer())
		{
			// swap desync on shot.
			if (g_menu.main.antiaim.invert_onshot.get())
...
void events::round_start(IGameEvent* evt) {
	...
	if (!strcmp(evt->GetName(), "round_start"))
	{
		...
		// update all players.
		// already called in FrameStage WTF
...
void events::round_end(IGameEvent* evt) {
	...
	if (!strcmp(evt->GetName(), "round_end"))
	{
...
void events::player_hurt(IGameEvent* evt) {
	...
	if (!strcmp(evt->GetName(), "player_hurt"))
	{
		int attacker, victim;
...
void events::bullet_impact(IGameEvent* evt) {
	...
	if (!strcmp(evt->GetName(), "bullet_impact"))
	{
...
void events::item_purchase(IGameEvent* evt) {
	...
	if (!strcmp(evt->GetName(), "item_purchase"))
	{
		int           team, purchaser;
		player_info_t info;
...
// Onetap v2 like Killstreaks going to be here >_<
void events::player_death(IGameEvent* evt) {
	...
	if (!strcmp(evt->GetName(), "player_death"))
	{
		...
		// healthshot >_<
		if (g_menu.main.misc.killeffect.get())
			g_cl.m_local->m_flHealthShotBoostExpirationTime() = g_csgo.m_globals->m_curtime + 1.0f;

		// push killstreaks
		g_reset.SetupPosition();
	}
}
...
void events::player_given_c4(IGameEvent* evt) {
	...
	if (!strcmp(evt->GetName(), "player_given_c4"))
	{
		player_info_t info;
...
void events::bomb_beginplant(IGameEvent* evt) {
	...
	if (!strcmp(evt->GetName(), "bomb_beginplant"))
	{
...
void events::bomb_abortplant(IGameEvent* evt) {
	...
	if (!strcmp(evt->GetName(), "bomb_abortplant"))
	{
...
void events::bomb_planted(IGameEvent* evt) {
	...
	if (!strcmp(evt->GetName(), "bomb_planted"))
	{
...
void events::bomb_beep(IGameEvent* evt) {
	...
	if (!strcmp(evt->GetName(), "bomb_beep"))
	{
...
void events::bomb_begindefuse(IGameEvent* evt) {
	...
	if (!strcmp(evt->GetName(), "bomb_begindefuse"))
	{
...
void events::bomb_abortdefuse(IGameEvent* evt) {
	...
	if (!strcmp(evt->GetName(), "bomb_abortdefuse"))
	{
...
void events::bomb_exploded(IGameEvent* evt) {
	...
	if (!strcmp(evt->GetName(), "bomb_exploded"))
	{
...
void events::bomb_defused(IGameEvent* evt) {
	...
	if (!strcmp(evt->GetName(), "bomb_defused"))
	{
		g_visuals.m_c4_planted = false;
		g_visuals.m_planted_c4 = nullptr;
	}
...
void events::round_freeze_end(IGameEvent* evt) {
	...
	if (!strcmp(evt->GetName(), "round_freeze_end"))
	{
...
void events::client_disconnect(IGameEvent* evt) {
	...
	if (!strcmp(evt->GetName(), "client_disconnect"))
	{
		g_cl.m_round_flags = 0;
	}
}
...
void Listener::init() {
	...
	add(XOR("player_hurt"), events::player_hurt); //- points to null on entity
	add(XOR("bullet_impact"), events::bullet_impact); //- always returns zero
	add(XOR("item_purchase"), events::item_purchase);
	add(XOR("player_death"), events::player_death); //- invalid entity
