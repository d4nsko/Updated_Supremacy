...
// forcing on BODY hitbox only not good at all
// no need to scan legs, arms, but feet and other, exclude neck
void AimPlayer::PreferHitbox()
{
	m_hitboxes.push_back({ HITBOX_PELVIS, HitscanMode::PREFER });
	//m_hitboxes.push_back({ HITBOX_BODY, HitscanMode::PREFER });	// nope don't use this
	m_hitboxes.push_back({ HITBOX_THORAX, HitscanMode::PREFER });
	m_hitboxes.push_back({ HITBOX_CHEST, HitscanMode::PREFER });
	m_hitboxes.push_back({ HITBOX_R_FOOT, HitscanMode::PREFER });
	m_hitboxes.push_back({ HITBOX_L_FOOT, HitscanMode::PREFER });
}
...
void AimPlayer::UpdateAnimations(LagRecord* record) {
	...
	if (m_player->m_AnimOverlay()[5].m_weight > 0.f)
...
// Forcing resolved hitboxes only
void AimPlayer::SetupHitboxes(LagRecord* record, bool history) {
	// reset hitboxes.
	m_hitboxes.clear();

	// rebuild. most conditions are cancer + by resolver logic, we don't need to scan two same point for body (for example body and pelvis)
	if (g_cl.m_weapon_id == ZEUS) {
		// hitboxes for the zeus.
		// best available if detected
	...
	// prefer, always, lethal, lethal x2
	// doesn't matter actually
	if (g_menu.main.aimbot.baim1.get(0) || g_menu.main.aimbot.baim1.get(1) || g_menu.main.aimbot.baim1.get(2))
		PreferHitbox();
	...
	// only, on key.
	// that's works much better this way, let it choose not only body, without head and neck
...
void Aimbot::find() {
	...
	for (const auto& t : m_targets) {
		...
		// history aim is possible at this point.
			...
			if (!t->GetBestAimPosition(last_pos, backtrack_damage, last, last_hitbox) && !SelectTarget(last, last_pos, backtrack_damage))
				continue;
	...
	if (best.player && best.record) {
		...
		// set autostop shit.
		m_stop = !(g_cl.m_buttons & IN_JUMP);	// we need to stop with minimum hitchance and penetration
		...
		bool hit = on && CheckHitchance(m_target, m_angle, m_record, best.hitbox) && can_hit_on_fd;

		// if we can scope.
		bool can_scope = /*!g_cl.m_local->m_bIsScoped()*/!scopeValue && (g_cl.m_weapon_id == AUG || g_cl.m_weapon_id == SG553 || g_cl.m_weapon_id == G3SG1 || g_cl.m_weapon_id == AWP || g_cl.m_weapon_id == SSG08 || g_cl.m_weapon_id == SCAR20);
	...
	// these hitboxes are capsules.
		...
		if (index == HITBOX_HEAD) {
			...
			if (g_menu.main.aimbot.multipoint.get(0)) {
				...
				if (state && record->m_anim_velocity.length() <= 0.1f && record->m_eye_angles.x <= state->m_flMinPitch) {
...
void Aimbot::apply() {
	...
	if (attack || attack2) {
		...
		if (m_target) {
			...
			if (g_menu.main.misc.hitbox_matrix.get() && iHit == true)	// only draw hitbox if shot was successfull
				g_visuals.DrawHitboxMatrix(m_record, { 255, 255, 255, 50 }, 10.f);
		}
		...
		// store fired shot.
		g_shots.OnShotFire(m_target ? m_target : nullptr, m_target ? m_damage : -1.f, g_cl.m_weapon_info->iBullets, m_target ? m_record : nullptr, m_hitbox);
