...
void Hooks::InitHooks( ) {
	...
	// Crashing at Animation state updating, net update and apply
	m_client.add( CHLClient::FRAMESTAGENOTIFY, util::force_cast( &Hooks::FrameStageNotify ) );	// Fixed

	m_client.add( CHLClient::USRCMDTODELTABUFFER, util::force_cast( &Hooks::WriteUsercmdDeltaToBuffer) );

	m_engine.init( g_csgo.m_engine );

	// Another debug points here
	m_engine.add( IVEngineClient::ISCONNECTED, util::force_cast( &Hooks::IsConnected ) );	// Fixed

	// This is too
	m_engine.add( IVEngineClient::ISHLTV, util::force_cast( &Hooks::IsHLTV ) );	// Fixed
	...
	// Visual norecoil crash
	m_prediction.add( CPrediction::INPREDICTION, util::force_cast( &Hooks::InPrediction ) );	// Fixed
	...
	// Too much and too often crashes in various states
	m_client_mode.add( IClientMode::CREATEMOVE, util::force_cast( &Hooks::CreateMove ) ); // All fixed
