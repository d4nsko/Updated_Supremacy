...
public:
	...
	__forceinline float& m_flHealthShotBoostExpirationTime() {
		return get< float >(g_entoffsets.m_flHealthShotBoostExpirationTime);
	}
...
public:
	...
	/*void SetNewAngle(vec3_t newAng)
	{
		typedef void(__thiscall* SetAngleFn)(void*, const vec3_t&);
		static SetAngleFn SetAngle = (SetAngleFn)((DWORD)pattern::find(g_csgo.m_client_dll, "55 8B EC 83 E4 F8 83 EC 64 53 56 57 8B F1"));
		SetAngle(this, newAng);
	}*/
...
public:
	bool			m_client_blend; //0x00
	float			m_blend_in; //0x04
	void*			m_studio_hdr; //0x08
	int				m_dispatched_src; //0x0C
	int				m_dispatched_dst; //0x10
	std::uintptr_t  m_order; //0x14
	std::uintptr_t  m_sequence; //0x18
	float			m_prev_cycle; //0x1C
	float			m_weight; //0x20
	float			m_weight_delta_rate; //0x24
	float			m_playback_rate; //0x28
	float			m_cycle; //0x2C
	void*			m_owner; //0x30
...
public:
	...
	__forceinline bool m_bIsLocalPlayer() {
		...
		return  g_csgo.m_engine->GetLocalPlayer() == this->index();
	}
	...
	__forceinline CCSGOPlayerAnimState *m_PlayerAnimState() {
		...
		return get< CCSGOPlayerAnimState * >(0x3914);
	}
...
public:
	...
	__forceinline void ModifyEyePosition(CCSGOPlayerAnimState *state, vec3_t *pos) {
		...
		if (state->m_baseEntity &&
			(state->m_bJust_Landed || state->m_baseEntity->m_flDuckAmount() != 0.f || !state->m_baseEntity->GetGroundEntity())) {
			auto v5 = 8;

			if (v5 != -1 && state->m_baseEntity->m_BoneCache().m_pCachedBones) {
				vec3_t head_pos(
	...
	__forceinline vec3_t GetShootPosition() {
		...
		if (*reinterpret_cast <int32_t *> (uintptr_t(this) + 0x3AC8)) {
	...
	__forceinline int GetSequenceActivity(int sequence) {
		...
		return g_csgo.GetSequenceActivity.as< GetSequenceActivity_t >()( this, GetModelPtr(), sequence );
	}
