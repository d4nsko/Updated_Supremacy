...
void Visuals::ModulateWorld() {
	...
	if (g_menu.main.visuals.world.get() == 1) {
		...
		for (const auto& w : skybox)
			w->ColorModulate(0.89f, 0.35f, 0.92f);
		...
		for (const auto& w : world)
			...
			w->ColorModulate(0.087f, 0.086f, 0.088f);
		...
		for (const auto& p : props)
			...
			p->ColorModulate(0.47f, 0.46f, 0.48f);
	}
...
// prevent force it after death (crashfix)
void Visuals::ThirdpersonThink() {
	...
	if (!g_csgo.m_engine->IsInGame() /*|| g_cl.m_local->m_iHealth() < 1*/)
		return;

	// check if we have a local player and he is alive.
	bool alive = g_cl.m_local && g_cl.m_local->m_iHealth() > 0;
...
void Visuals::Hitmarker() {
	...
	constexpr int line{ 12 };

	// fat
	render::line(x - line, y - line, x - (line / 4), y - (line / 4), { 255, 255, 255, alpha });
	render::line(x - line, y + line, x - (line / 4), y + (line / 4), { 255, 255, 255, alpha });
	render::line(x + line, y + line, x + (line / 4), y + (line / 4), { 255, 255, 255, alpha });
	render::line(x + line, y - line, x + (line / 4), y - (line / 4), { 255, 255, 255, alpha });

	// damage indicator above head
	std::string out = tfm::format(XOR("- %i health\n"), iHitDmg);

	if (render::WorldToScreen(iPlayermins, iPlayerbottom) && render::WorldToScreen(iPlayermaxs, iPlayertop))
	{
		// get the esp box info >_<
		Rect box;
		box.h = iPlayerbottom.y - iPlayertop.y;
		box.w = box.h / 2.f;
		box.x = iPlayerbottom.x - (box.w / 2.f);
		box.y = iPlayerbottom.y - box.h;

		// text damage
		render::hud.string(box.x + box.w / 2, box.y - render::esp.m_size.m_height - 10, { 255, 0, 0, alpha }, out, render::ALIGN_CENTER);

		if(iHeadshot == true)
			render::hud.string(box.x + box.w / 2, box.y - render::esp.m_size.m_height - 20, { 255, 180, 30, alpha }, "headshot", render::ALIGN_CENTER);
	}
}
...
void Visuals::SpreadCrosshair() {
	...
	extern float fov;
	// calc radius.
	float radius = ((weapon->GetInaccuracy() + weapon->GetSpread()) * 320.f) / (std::tan(math::deg_to_rad(fov) * 0.5f) + FLT_EPSILON);
...
// this just crashes at update
// or at local player death, depended on how many players are on server
// if too much rendered arrows - it will crash immediately or on update >_<
...
bool IsFakeDucking(Player* player) {
	static int storedTick = 0;
	static int crouchedTicks[64] = {};
	...
	if (storedTick != g_csgo.m_globals->m_tick_count)
	{
		crouchedTicks[player->index()]++;
		storedTick = g_csgo.m_globals->m_tick_count;
	}

	if (duckspeed == 8 && duckamount <= 0.9 && duckamount > 0.01 
		&& (player->m_fFlags() & FL_ONGROUND) && (crouchedTicks[player->index()] >= 5))
		return true;
	else
		return false;
}
...
void Visuals::DrawPlayer(Player* player) {
	...
	// draw flags.
	{
		...
		for (auto it = items.begin(); it != items.end(); ++it) {
			...
			// money.
			if (*it == 0)
				flags.push_back({ tfm::format(XOR("%i BUCKS"), player->m_iAccount()), { 255, 255, 255, low_alpha } });

			// armor.
			if (*it == 1) {
				// helmet and kevlar.
				if (player->m_bHasHelmet() && player->m_ArmorValue() > 0)
					flags.push_back({ XOR("KEVLAR & HELMET"), { 255, 255, 255, low_alpha } });

				// only helmet.
				else if (player->m_bHasHelmet())
					flags.push_back({ XOR("HELMET"), { 255, 255, 255, low_alpha } });

				// only kevlar.
				else if (player->m_ArmorValue() > 0)
					flags.push_back({ XOR("KEVLAR"), { 255, 255, 255, low_alpha } });
			}

			// scoped.
			if (*it == 2 && player->m_bIsScoped())
				flags.push_back({ XOR("ZOOMING"), { 60, 180, 225, low_alpha } });

			// flashed.
			if (*it == 3 && player->m_flFlashDuration() > 0.f)
			...
			if (*it == 5 && player->HasC4())
				flags.push_back({ XOR("BOMB CARRIER"), { 255, 0, 0, low_alpha } });

			if (*it == 6 && IsFakeDucking(player)) {
				flags.push_back({ XOR("FAKEDUCKING"), { 255, 0, 0, low_alpha } });
			}

			// resolver info
			if (*it == 7 && iPlayers[player->index()] == true)
				flags.push_back({ XOR("RESOLVER: DETECTED"), { 153, 153, 255, low_alpha } });
			if (*it == 7 && iPlayers[player->index()] == false)
				flags.push_back({ XOR("RESOLVER: UNKNOWN"), { 153, 153, 255, low_alpha } });

			// hp info
			if (*it == 8 && player->m_iHealth() > 0)
				flags.push_back({ tfm::format(XOR("HEALTHBAR: %i HP"), player->m_iHealth()), { 178, 255, 102, low_alpha } });
		}
		...
		for (size_t i{ }; i < flags.size(); ++i) {
			...
			// -3 or -1
			int offset = i * (render::esp_small.m_size.m_height - 3);

			// draw flag.
			render::esp_small.string(box.x + box.w + 4, box.y + offset, f.second, f.first);
		}
	}
			...
			if (weapon) {
				...
				if (data) {
					...
					else if (g_menu.main.players.weapon_mode.get() == 2)
					{
						...
						// letters, this is fine
						offset += 22;

						// smallfonts needs upper case.
						std::transform(name.begin(), name.end(), name.begin(), ::toupper);

						render::esp_small.string(box.x + box.w / 2, box.y + box.h + offset, { 255, 180, 30, low_alpha }, name, render::ALIGN_CENTER);
					}
				}
			}
		}
