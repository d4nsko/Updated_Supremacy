...
void AquaWalk()
{
	if (g_cl.m_cmd->m_forward_move > NULL)
		g_cl.m_cmd->m_buttons |= IN_BACK, g_cl.m_cmd->m_buttons &= ~IN_FORWARD;

	if (g_cl.m_cmd->m_forward_move < NULL)
		g_cl.m_cmd->m_buttons |= IN_FORWARD, g_cl.m_cmd->m_buttons &= ~IN_BACK;

	if (g_cl.m_cmd->m_side_move < NULL)
		g_cl.m_cmd->m_buttons |= IN_MOVERIGHT, g_cl.m_cmd->m_buttons &= ~IN_MOVELEFT;

	if (g_cl.m_cmd->m_side_move > NULL)
		g_cl.m_cmd->m_buttons |= IN_MOVELEFT, g_cl.m_cmd->m_buttons &= ~IN_MOVERIGHT;
}
...
void Movement::FixMove(CUserCmd* cmd, ang_t& wish_angles) {
	...
	// disabled both callback condition
	if (!g_menu.main.movement.slide_on_stop.get())
	{
		if (g_cl.m_local->m_MoveType() != MOVETYPE_LADDER)
			cmd->m_buttons &= ~(IN_FORWARD | IN_BACK | IN_MOVERIGHT | IN_MOVELEFT);
	}

	// slide only while zooming
	if(g_menu.main.movement.slide_on_stop.get() && scopeValue)
		AquaWalk();
...
void Movement::QuickStop() {
	...
	if (g_cl.m_weapon_info) {
		...
		float flMaxSpeed =/* g_cl.m_local->m_bIsScoped()*/scopeValue ? g_cl.m_weapon_info->flMaxPlayerSpeedAlt : g_cl.m_weapon_info->flMaxPlayerSpeed;
