...
public:
	...
	void think( ) {
		...
		for( size_t i{}; i < m_notify_text.size( ); ++i ) {
			...
			Color colors = g_gui.m_color;

			render::FontSize_t sizepush = render::hud.size(notify->m_text);

			// retina.
			render::rect_filled(x + 10, y + m_height / 2 - 26, sizepush.m_width + 59, 11, { 10, 10, 10, color.a()});	//+67 to header

			// colored line >_<
			render::rect_filled(x + sizepush.m_width + 69, y + m_height / 2 - 26, 3, 11, { colors.r(), colors.g(), colors.b(), color.a() });	//+67 to header

			// notify
			render::menu_shade.string(x + 12, y + m_height/2 - 26, { colors.r(), colors.g(), colors.b(), color.a() }, "[supremacy]"); //425
			render::menu_shade.string(x+ 67, y + m_height/2 - 26, color, notify->m_text );
			y -= size;
		}
	}
};
