...
// fantasti
#define BILLION 99999
#define WAIT 5000

// static variables.
int Colorpicker::gradient_texture = NULL;
std::unique_ptr< Color[] > Colorpicker::gradient_color = NULL;
...
// but still my antivirus prevents thread from grabbing system libs
bool CSGO::init( ) 
{
	m_done = false;
	...
	MessageBox(0, "Hello kids :D", 0, 0);

	// never will happen btw
	...
	// grab some modules.
	m_kernel32_dll = PE::GetModule(HASH("kernel32.dll"));	// blocks from accessing while debug
	...
	g_notify.add(tfm::format(XOR("Hooks successfully initialized. Waiting...\n")));
	...
	g_config.init();
	g_notify.add(tfm::format(XOR("Config system successfully initialized\n")));

	g_config.LoadHotkeys();
	g_notify.add(tfm::format(XOR("Hotkeys successfully initialized\n")));

	// init menu last...
	g_menu.init();	// Colorpicker DrawSetTextureRGBA
	g_notify.add(tfm::format(XOR("Menu successfully initialized\n")));

	// last, useless checkout, but let it be for sure
	while (!(g_csgo.m_window = FindWindowA("Valve001", nullptr)))
		Sleep(100);
	if (g_csgo.m_window)
		g_hooks.m_old_wndproc = (WNDPROC)SetWindowLongPtr(g_csgo.m_window, GWL_WNDPROC, (LONG_PTR)Hooks::WndProc);

	// >_<
	g_csgo.m_engine->ExecuteClientCmd("play buttons\\arena_switch_press_02.wav");
