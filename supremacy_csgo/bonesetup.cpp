...
bool Bones::BuildBones( Player* target, int mask, BoneArray* out, LagRecord* record ) {
	...
	// why?
	static int bSkipAnimationFrame = *reinterpret_cast< int* >( uintptr_t( target ) + 0x274 );
	*reinterpret_cast< int* >( uintptr_t( target ) + 0x274) = NULL;
	...
	*reinterpret_cast<int*>(uintptr_t(target) + 0x274) = bSkipAnimationFrame;
