...
void Colorpicker::init( ) {
	...
	// allocate.
	gradient_color = std::make_unique< Color[] >( w * h );
	...
	for( int i{}; i < w; ++i ) {
		...
		for( int j{}; j < h; ++j ) {
			...
			// write back to array.
			*( Color* )( gradient_color.get( ) + i + j * w ) = Color::hsl_to_rgb( hue, sat, lum );
	...
	if(!gradient_texture)
		gradient_texture = g_csgo.m_surface->CreateNewTextureID( true );

	// assign allocated memory containing the picker to said texture.
	if (gradient_texture && gradient_color)
		g_csgo.m_surface->DrawSetTextureRGBA(gradient_texture, { gradient_color.get() }, w, h);
}
...
void Colorpicker::draw( ) {
	...
	if( m_open ) {
		...
		// set texture.
		g_csgo.m_surface->DrawSetTexture(gradient_texture);
